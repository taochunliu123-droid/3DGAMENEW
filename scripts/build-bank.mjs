...
const seen=new Set(),bank=[],missingEn=[];let errors=0;
...
for(const file of walk(base).sort()){
  ...
  blocks.forEach((block,n)=>{
    ...
    if(new Set(['A','B','C','D'].map(k=>sections[k])).size!==4)return fail(where,'四個中文選項不可重複');
    const sig=audience+'\n'+sections['情境']+'\n'+sections['問題'];if(seen.has(sig))return fail(where,'與其他題目的情境和問題完全相同');seen.add(sig);
    // 英文章節可省略：省略時英文模式顯示中文，並在建置時提醒
    const EN={'情境':'Scenario','問題':'Question','A':'A (EN)','B':'B (EN)','C':'C (EN)','D':'D (EN)','解析':'Why','提示':'Hint'};
    const lacking=Object.values(EN).filter(k=>!sections[k]);
    if(lacking.length){missingEn.push(where+'（缺 '+lacking.join('、')+'）')}
    const enOpts=['A','B','C','D'].map(k=>sections[k+' (EN)']).filter(Boolean);
    if(enOpts.length===4&&new Set(enOpts).size!==4)return fail(where,'四個英文選項不可重複');
    const bi=k=>({zh:sections[k],en:sections[EN[k]]||sections[k]});
    const [tz,te]=title.split(/\s*\|\s*/);
    bank.push({id:`${audience}-s${stage}-${String(n+1).padStart(2,'0')}`,audience,stage,answer:ans[1].toLowerCase(),
      title:{zh:tz,en:te||tz},scenario:bi('情境'),question:bi('問題'),
      options:['a','b','c','d'].map(id=>({id,text:bi(id.toUpperCase())})),
      explanation:bi('解析'),hint:bi('提示')});
  });
}
...
if(missingEn.length){console.warn('提醒：'+missingEn.length+' 題的英文還沒寫完，英文模式會先顯示中文：');missingEn.slice(0,10).forEach(m=>console.warn('  '+m))}
